...
def extract_homonym_sentences(filename, homonyms, homonym_sentences):
    ...
    for sentence_parsed in root.iter('{http://www.tei-c.org/ns/1.0}s'):
        ...
        for word_xml in sentence_parsed:
            ...
            if "lemma" in word_xml.attrib:
                ...
                if word_xml.attrib["lemma"] in homonyms:
                    lemma_start_index = len(lemma_sentence)
                    lemma_end_index = lemma_start_index + len(word_xml.attrib["lemma"]) + 1
        ...
        if contains_homonym_lemma:
            sentence_entry = SentenceEntry(sentence, lemma_sentence, start_index, end_index, lemma_start_index,
                                           lemma_end_index)
...
class OutputEntry:
    def __init__(self, word, sentence1_object, sentence2_object):
        ...
        self.lemma_start1 = sentence1_object.lemma_start
        self.end1 = sentence1_object.end
        self.lemma_end1 = sentence1_object.lemma_end
        self.start2 = sentence2_object.start
        self.lemma_start2 = sentence2_object.lemma_start
        self.end2 = sentence2_object.end
        self.lemma_end2 = sentence2_object.lemma_end
        self.same_context = False


class SentenceEntry:
    def __init__(self, sentence, lemma_sentence, start, end, lemma_start, lemma_end):
        self.sentence = sentence
        self.lemma_sentence = lemma_sentence
        self.start = start
        self.end = end
        self.lemma_start = lemma_start
        self.lemma_end = lemma_end
